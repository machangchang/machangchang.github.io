...
```java
/**
     * 一笔有利的交易至少需要两天（在一天买入，在另一天卖出，前提是买入价格低于卖出价格）。如果 prices 数组的长度为 n ，则最大利润交易次数为 n/2
     * （整数除法）。之后将不再可能进行有利交易，这意味着最大利润将保持不变（重点：因为最大利润不变了，所以 k 可以无限大！！！，和 K 等于无限大的利润相同）。 因此， k 的临界值为 n/2 。如果给定的 k 不小于这个值，即
     * k >= n/2 ，则可以将 k 扩展到正无穷大，问题就等同于 K不限次数 。
     */
    // 三维数组解法
    public int maxProfit(int k, int[] prices) {
        int n = prices.length;
